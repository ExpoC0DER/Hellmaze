...
namespace _game.Scripts
{
    public class MazeNode : MonoBehaviour
    {
        ...
        void SetProbabilities(MapSettings mapSettings)
        {
            int lastMax = 0;

            _fullWallRange.x = 0;
            _fullWallRange.y = lastMax += (int)mapSettings.FullWallProb;
            _bottomHoleRange.x = _fullWallRange.y;
            _bottomHoleRange.y = lastMax += (int)mapSettings.CrouchSpaceProb;
            _topHoleRange.x = _bottomHoleRange.y;
            _topHoleRange.y = lastMax += (int)mapSettings.GrapplingHookProb;
            _breakableWallRange.x = _topHoleRange.y;
            _breakableWallRange.y = lastMax += (int)mapSettings.DestructableWallProb;
            _glassWallRange.x = _breakableWallRange.y;
            _glassWallRange.y = lastMax += (int)mapSettings.GlassWallProb;

            _noWallRange.x = (int)mapSettings.WallProb;
            _noWallRange.y = 100;

            //Debug.Log("setup full " + _fullWallRange + "\n" + "no " + _noWallRange + "\n" + "bottom " + _bottomHoleRange + "\n" + "top  " + _topHoleRange +  "\n" + "break " + _breakableWallRange + "\n" + "glass " + _glassWallRange);
            //Debug.Log("_settingsSetter full " + _settingsSetter.FullWall_prob + "\n" + "yes " + (int)_settingsSetter.Wall_prob + "\n" + "bottom " + (int)_settingsSetter.CrouchSpace_Prob + "\n" + "top  " + (int)_settingsSetter.GrapplingHook_Prob +  "\n" + "break " + (int)_settingsSetter.DestructableWall_Prob + "\n" + "glass " + (int)_settingsSetter.GlassWall_prob);
        }
